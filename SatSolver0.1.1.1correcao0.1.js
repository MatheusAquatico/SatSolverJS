...
function readclauses(text) {
  ...
  for (w = 0; w < clauses.length; w++) {
      ...
      clauses[w] = clausesAux; // o arrayClauses recebe um array bidimensional com todas as clausulas e todas as variáveis num array próprio.
        
      clausesAux = []; // Apaga a cláusula atual
  ...
  function readVariables(clauses) {
    ...
    for(let d = 0; d<clauses.length; d++) {
      let clause = clauses[d];
   
      for(let j = 0; j<clause.length; j++) {
        let arrayVariable = clause[j];
   
         
        if(arrayVariable < 0) {
          arrayVariable = -arrayVariable;   // para caso de negação
        }
   
        
        if(!arrayVariables.includes(arrayVariable)) { //Faz com que a variável seja adicionada se ela ainda não constar
          arrayVariables.push(arrayVariable);
        }
      }
    }
   
    arrayVariables.sort((a, b) => a - b);// Ordena 
